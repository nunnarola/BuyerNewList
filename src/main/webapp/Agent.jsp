...
sHeadFileIncludes = sHeadFileIncludes + "<link href='/styles/admin.css?" + v + "' rel='stylesheet' />";
sHeadFileIncludes = sHeadFileIncludes + "<link href='/styles/admin_forms.css?" + v + "' rel='stylesheet' />";
sHeadFileIncludes = sHeadFileIncludes + "<script type='text/javascript' src='/js/admin.js?" + v + "'></script>";

sHeadFileIncludes = sHeadFileIncludes + "<script type='text/javascript' src='/js/plupload.full.min.js?" + v + "'></script>";
sHeadFileIncludes = sHeadFileIncludes + "<script type='text/javascript' src='/js/user.js?" + v + "'></script>";
sHeadFileIncludes = sHeadFileIncludes + "<script type='text/javascript' src='/js/jquery.plupload.queue/jquery.plupload.queue.min.js' charset='UTF-8'></script>";



bNeedAuth = true;   //page needs authentication 
bRememberedIsOK = false;
bShowLogoinTB = true;
bShowFooterContact = false;
...
if (bMustBeSecure && bProd &&  !request.isSecure()){  if (!bSentRedir){response.sendRedirect(sSecureServer + request.getRequestURI());bSentRedir = true;}}
...
if (request.getParameter("act") == null && id != null ){ // just viewing
	...
	if (theObject != null && theObject.getId() > 0){
		...
		if (theObject.isLocked()){inactive = true;}
...
%>
	...
	<div class="pghead">
		...
		<FORM METHOD="POST" NAME="ADDPROP" ACTION='<%= lm.GetSecureServer()%>/Admin/<%= objectName %>.jsp' autocomplete="off" id="leadentryform" accept-charset="ISO-8859-1">
			...
			<div id="leadlistholder" style="padding-top:30px;">
				...
				<div >
					...
					<div class='addleadpopform'>
						...
						<%} %>
						<div class="editleadstatus" <% if (!"".equals(sError) || updatedOK || addedOK){out.print(" style='display:inline-block;'");}%>>
						
						<%if (updatedOK){
							
							%><div class="leadupok"><i class="fa fa-check-circle-o" aria-hidden="true"></i>&nbsp;&nbsp;UPDATE OK<BR><a href='/Admin/<%= objectName %>s.jsp' style='color:white;'>Go to <%= objectName %>s</a></div><%

						}else if (addedOK){
							
							%><div class="leadupok"><i class="fa fa-check-circle-o" aria-hidden="true"></i>&nbsp;&nbsp;ADDED OK<BR><a href='/Admin/<%= objectName %>s.jsp' style='color:white;'>Go to <%= objectName %>s</a></div><%

						}else{
							
							%><div class="leadupfail"><%=sError %></div><% 
						}%>
...
%>
		...
		*** The picture is uploaded to the loud but only associated to this record when agent object is saved ***/
		...
		%>
		<input type="hidden" value="<%= currentImageValue %>" name="<%= imageURLInputId %>" id="<%= imageURLInputId %>">		
							...
							<div class='verval'>
								...
								<div id='profpic'>
									...
									<span class="prfldholder"
										<% if(bGotPic){out.print(" style='display:none;'");}%>><i
										class='fa fa-user' aria-hidden='true'></i></span>
								</div>
								<span class="chgpg" id="chpcbt" onclick="showpicloadr();">Upload</span>
								<span class="chgpg" id="delpcbt" onclick="quickDelPic('<%= imageURLInputId %>');" <% if(!bGotPic){out.print(" style='display:none;'");}%>>Delete</span>
							</div>
							<span class="upok" style="display: inline-block;" id="profpires"><i
								class='fa fa-check-circle' aria-hidden='true'
								<% if(!bGotPic){out.print(" style='display:none;'");}%>></i></span>
			...
			<div class="AdInputInfo"><span class="aperm"><%= imageError %></span></div>
						...
						<div class='adldfieldhld'>
							<div class='addleadpopbtnwrap' >
								<input type="submit" name="sendbutt" value="<%= actionButtonTitle %>" class="leadpopbtn" >
							</div>
						</div>
