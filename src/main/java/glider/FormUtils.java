...
String errorClass = "";
if (!"".equals(common.nnt(errorMessage))) {errorClass = "attention";}
...
/**
	...
	**/
	...
	if (fieldType == 2) {fieldTypeHtml = "date";}
	
	String currencyHtml = "";
	if (fieldType == 1) {currencyHtml = "<span class='currency'>R</span>";}
...
try {
	...
	String errorClass = "";
	if (!"".equals(common.nnt(errorMessage))) {errorClass = "inerr";}
	...
	for (String key:values.keySet()) {
		...
		if ("".equals(key)) {gotEmptyStringAsOtion = true;}
		options.append(">" +values.get(key) + "</option>");	
		
	}
	// hasSelectRequest means teh select list already has a 'Select...' at the top
	if (!hasSelectRequest && !gotEmptyStringAsOtion) {r.append("<option value=''>Select...</option>") ;}
	r.append(options);
	r.append("</select>");
	r.append("</div>");
	r.append(subTextHtml);
